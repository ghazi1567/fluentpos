...
#pragma warning restore RCS1046 // Asynchronous method name should end with 'Async'.
        {
            Expression<Func<Category, GetCategoriesResponse>> expression = e => new GetCategoriesResponse(e.UUID, e.Name, e.Detail);
...
#pragma warning restore RCS1046 // Asynchronous method name should end with 'Async'.
        {
            var category = await _context.Categories.AsNoTracking().Where(c => c.UUID == query.Id).FirstOrDefaultAsync(cancellationToken);
            if (category == null)
            {
...
#pragma warning restore RCS1046 // Asynchronous method name should end with 'Async'.
        {
            string data = await _context.Categories.AsNoTracking().Where(c => c.UUID == request.Id).Select(a => a.ImageUrl).FirstOrDefaultAsync(cancellationToken);
