...
export class ReturnOrderComponent implements OnInit {
    ...
    constructor(private orderService: OrdersService, private fb: FormBuilder, private toastr: ToastrService) {}
    ...
    getOrder(orderNo) {
        this.orderService.getByTrackingNumber(orderNo).subscribe((response) => {
    ...
    returnOrder() {
        var isConditionSelected = true;
        this.confirmedItems.forEach((x) => {
            if (!x.condition) {
        ...
        if (!isConditionSelected) {
            this.toastr.error("Please product condition");
            return;
        }
        ...
        this.isSppiner = true;
        this.orderService.returnOrder(model).subscribe((response) => {
