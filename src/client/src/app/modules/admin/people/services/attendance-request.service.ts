...
export class AttendanceRequestService {
    ...
    createShiftPlan(model: any) {
        return this.api.createShiftPlan(model).pipe(map((response: IResult<any>) => response));
    }

    deleteShiftPlan(id: string) {
        return this.api.deleteShiftPlan(id).pipe(map((response: IResult<string>) => response));
    }

    getShiftPlans(searchParams: PeopleSearchParams): Observable<PaginatedResult<any>> {
        let params = this.getParams(searchParams);
        return this.api.getShiftPlans(params).pipe(map((response: PaginatedResult<any>) => response));
    }
