...
export class CronGenComponent implements OnInit, ControlValueAccessor {
    ...
    private computeDailyCron(state: any) {
        switch (state.subTab) {
            case "everyDays":
                this.cron = `${this.isCronFlavorQuartz ? state.everyDays.seconds : ""} ${state.everyDays.minutes} ${this.hourToCron(state.everyDays.hours, state.everyDays.hourType)} 1/${
                    state.everyDays.days
                } * ${this.weekDayDefaultChar} ${this.yearDefaultChar}`.trim();
                break;
            case "everyWeekDay":
                this.cron = `${this.isCronFlavorQuartz ? state.everyWeekDay.seconds : ""} ${state.everyWeekDay.minutes} ${this.hourToCron(state.everyWeekDay.hours, state.everyWeekDay.hourType)} ${
    ...
    private computeWeeklyCron(state: any) {
        ...
        this.cron = `${this.isCronFlavorQuartz ? state.seconds : ""} ${state.minutes} ${this.hourToCron(state.hours, state.hourType)} ${this.monthDayDefaultChar} * ${days} ${
    ...
    private computeMonthlyCron(state: any) {
        switch (state.subTab) {
            case "specificDay":
                this.cron = `${this.isCronFlavorQuartz ? state.specificDay.seconds : ""} ${state.specificDay.minutes} ${this.hourToCron(state.specificDay.hours, state.specificDay.hourType)} ${
                    state.specificDay.day
                } 1/${state.specificDay.months} ${this.weekDayDefaultChar} ${this.yearDefaultChar}`.trim();
                break;
            case "specificWeekDay":
                this.cron = `${this.isCronFlavorQuartz ? state.specificWeekDay.seconds : ""} ${state.specificWeekDay.minutes} ${this.hourToCron(
                    state.specificWeekDay.hours,
                    state.specificWeekDay.hourType
                )} ${this.monthDayDefaultChar} 1/${state.specificWeekDay.months} ${state.specificWeekDay.day}${state.specificWeekDay.monthWeek} ${this.yearDefaultChar}`.trim();
                break;
            default:
    ...
    private computeYearlyCron(state: any) {
        switch (state.subTab) {
            case "specificMonthDay":
                this.cron = `${this.isCronFlavorQuartz ? state.specificMonthDay.seconds : ""} ${state.specificMonthDay.minutes} ${this.hourToCron(
            ...
            case "specificMonthWeek":
                this.cron = `${this.isCronFlavorQuartz ? state.specificMonthWeek.seconds : ""} ${state.specificMonthWeek.minutes} ${this.hourToCron(
                    state.specificMonthWeek.hours,
                    state.specificMonthWeek.hourType
                )} ${this.monthDayDefaultChar} ${state.specificMonthWeek.month} ${state.specificMonthWeek.day}${state.specificMonthWeek.monthWeek} ${this.yearDefaultChar}`.trim();
                break;
            default:
