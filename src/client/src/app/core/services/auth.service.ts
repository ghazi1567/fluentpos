...
import { BranchApiService } from "../api/org/branch-api.service";
...
export class AuthService {
    ...
    constructor(private http: HttpClient, private localStorage: LocalStorageService, private router: Router, private toastr: ToastrService, private branchApiService: BranchApiService) {}
    ...
    public get getBranchs(): any {
        ...
        if (decodedToken && decodedToken.branch) {
            if (Array.isArray(decodedToken.branch)) {
                decodedToken.branch.forEach((x) => {
                    tempBranchs.push(JSON.parse(x));
                });
            } else {
                tempBranchs.push(JSON.parse(decodedToken.branch));
            }
        }
        tempBranchs.forEach((x) => {
            this.branchApiService.getById(x.BranchId).subscribe((x) => {
                branchs.push(x.data);
                this.localStorage.setItem("branchs", JSON.stringify(branchs));
            });
        });
    ...
    public get getIpAddress(): string {
        const decodedToken = this.getDecodedToken();
        return !!decodedToken ? decodedToken["ipAddress"] : "";
    }
    ...
    public tryRefreshingToken(): void {
        ...
        this.http
            .post(this.baseUrl + "identity/tokens/refresh", {
